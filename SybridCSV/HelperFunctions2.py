...
def get_percentages(brand_overall, cat=2, printit=False):
    i=0
    b = []
    s = []
    brandlabels=[]
...
def store_data5(filename,Sentiment,brand,averagesent,ifquery):
    # Storing to csv File
    # Sentiment gives file with percentages fof sentiment in brand
    # brand is unique brands
    # avearge sent gives a score between -100/100
    # if query gives percentage of sentiment vs query
    with open(filename, 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['Brand','Percent Sentiment','Percent Very Positive', 'Percent Positive',
                         'Percent Neutral','Percent Negative', 'Percent Very Negative',
                         'Percent Query', 'Overall Score'])
        for i in range(np.size(brand)):
            q0 = float("{0:.2f}".format(ifquery[i][0]))
            q1 = float("{0:.2f}".format(ifquery[i][1]))
            avsent = averagesent[brand[i]]
            s1 = float("{0:.2f}".format(Sentiment[i][0]))
            s2 = float("{0:.2f}".format(Sentiment[i][1]))
            s3 = float("{0:.2f}".format(Sentiment[i][2]))
            s4 = float("{0:.2f}".format(Sentiment[i][3]))
            s5 = float("{0:.2f}".format(Sentiment[i][4]))
            
            writer.writerow([brand[i],q1,s1,s2, s3,s4,s5,q0, avsent])
    file.close()

#nltk.download('stopwords')
def brand_sentiment2(brand, features, svm):
    ...
    for i in range(np.size(tags)):
        ...
        brand_overall[tags[i],'Query']=0
        brand_overall[tags[i],'Sentiment']=0
    ...
    for key, sent in brand_overall:
        brand_overall[key,sent]= 100*brand_overall[key,sent]/brand_instances[key]
        
    return brand_overall



def brand_sentiment3(brand, features, svm,n,plot=True):
    preds = svm.predict(features)
    tags = np.unique(brand)
    ...
    brand_instances={}
    brand_overall={}
    brand_instances_sent={}
    for i in range(np.size(tags)):
        brand_data_sentiment[tags[i]]=0
        brand_instances_sent[tags[i]]=0
        brand_instances[tags[i]]=0
        
        brand_overall[tags[i],'Positive']=0
        brand_overall[tags[i],'Neutral']=0
        brand_overall[tags[i],'Negative']=0
    
    for j in range(np.size(preds)):
        brand_instances[brand[j]]+=1
        brand_instances_sent[brand[j]]+=1
        if preds[j] == 'Positive':
    ...
    return brand_data_sentiment, brand_overall




def brand_sentiment5(brand, features, svm,n,plot=True):
    preds = svm.predict(features)
    tags = np.unique(brand)
    ...
    for i in range(np.size(tags)):
        ...
        brand_overall[tags[i],'Very Positive']=0
        brand_overall[tags[i],'Positive']=0
        brand_overall[tags[i],'Neutral']=0
        brand_overall[tags[i],'Negative']=0
        brand_overall[tags[i],'Very Negative']=0
...
def load_data_nolabels(filenames,shift=True):
    ###
    # Inputs:
    #    # filename: (string: csv file name)
    #    # num_annotate: (integer: 1 or 2) determines if one or two sets of labels are present
    ...
    for i in range(np.size(filenames)):
        ...
        if i==0:
            ...
            Tweetsall = np.concatenate((Tweetsall,Tweets),axis=0)
            brandsall = np.concatenate((brandsall,brand),axis=0)
                   
    all_stopwords = return_stopwords(lists=[],append=True)
    cleaned =clean_data(Tweetsall,brandsall,all_stopwords)
    
    return Tweetsall,brandsall,cleaned
